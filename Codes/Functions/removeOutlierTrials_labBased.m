function ha = removeOutlierTrials_labBased(ha_org,i_newblock)
...
for s=1:nS
    ...
    trial_out1=find(abs(ha_withOutlierTrials)>100); % base on absolute hand angle
    if abs(ha_withOutlierTrials(1))>25 % when the first trial has too large error
